public class Dictionary {
    ...
    void choose_pivot_line(){
        float test_min = 0;
        float min = this.constraint_matrix[0][this.dictionary_number_variables]/this.constraint_matrix[0][this.input_index];
        this.output_index = 0; 
        for(int i = 1; i < number_constraints; i++ ){
            test_min = this.constraint_matrix[i][this.dictionary_number_variables]/this.constraint_matrix[i][this.input_index];
            if(test_min < min){
                min = test_min;
                this.output_index = i;
            }
        ...
        for (int i = 0; i < number_constraints; i++){
            if(i >= this.output_index && i <= this.output_index){
                ...
                for(int j = 0; j < nbcol; j++){
                    this.constraint_matrix[i][j] = this.constraint_matrix[i][j] - factor * this.constraint_matrix[this.output_index][j] ;
                }
            }
        }
